<template>
    <div class="programme">
      ...
      <div class="Programme_con">
        ...
        <div class="list">
          <h2><i class="icon iconfont icon-chakanjindu"></i>方案进度 </h2>
          <div class="Percentage green">
            <div class="Percentage_num" :style="{width: presentProgramme.schedule+'%'}">{{ presentProgramme.schedule }}%</div>
          </div>
        </div>
        ...
        <div class="start_btn" v-if="presentProgramme.isFinshed != 2">
          <span class="run_time" v-if="presentProgramme.isFinshed == 1">正在进行中...</span>
          <button class="Go_finish" @click="execute" v-if="presentProgramme.isFinshed != 1">去完成</button>
          <button @click="complete">今日已完成</button>
        </div>
      </div>
...
export default {
    ...
    methods:{
      execute () {
        this.presentProgramme.isFinshed = 1; // 当日是否完成标志2已完成，  1已开始， 0未开始
        this.$http.post("/healthFront/out/index/goFinishScheme.do", this.Qs.stringify({
          schemeMemberId: this.$route.query.id
        }))
      },
      complete () {
        this.presentProgramme.isFinshed = 2;
        this.$http.post("/healthFront/out/index/finishScheme.do", this.Qs.stringify({
          schemeMemberId: this.$route.query.id
        }));
        this.$router.push({
          path:'/projectManage'
